                     ...
                     OverSampling      ovs)
{
    ...
    if(oversampling_)
    {
        ...
        {
            // Note: HAL_ADC_Init() encodes OVSR as (Ratio - 1) for STM32H7,
            // so Ratio here is the true oversample count N. Passing N-1 (the
            // raw register value) produces N-1 samples summed with shift-by-N
            // division, yielding a systematic (N-1)/N gain error (e.g. -3.125%
            // at OVS_32). Pass N directly.
            case OVS_4:
